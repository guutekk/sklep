<?php
    ...
    if(isset($_POST['submit_sortowanie_cena']))
    {
        ...
        if($value=='1')
        {
            $sql="SELECT * FROM products ORDER BY Cena ASC";
            $result = mysqli_query($connect, $sql);
        }
        else if($value=='2')
        {
            $sql="SELECT * FROM products ORDER BY Cena DESC";
            $result = mysqli_query($connect, $sql);
        }
        else if($value=='3')
        {
    ...
    if(isset($_POST['submit_sortowanie_nazwa']))
    {
        ...
        if($value=='1')
        {
            $sql="SELECT * FROM products ORDER BY Nazwa ASC";
            $result = mysqli_query($connect, $sql);
        }
        else if($value=='2')
        {
            $sql="SELECT * FROM products ORDER BY Nazwa DESC";
            $result = mysqli_query($connect, $sql);
        }
        else if($value=='3')
        {
        ...
        <div class="Items">
            <?php
                for($i=0; $i<mysqli_num_rows($result); $i++)
                {
                    ...
                    <form method='POST' action='Koszyk_modyfikacje.php?mode=dodaj&id_produktu=$row[Id_produktu]&ilosc=1'>
